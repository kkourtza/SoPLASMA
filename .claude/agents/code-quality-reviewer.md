...
```
Quote no fixed total — it moves (106 over those two dirs, 174 over all of `src/` at HEAD `aed5dbe`). For each hit **in the diff**, read ±20 lines: is it inside `if (pf.size())`, `if (patch()...size())`, or downstream of an early `return` keyed on a LOCAL patch/cell count? Every rank must reach a collective the same number of times.
