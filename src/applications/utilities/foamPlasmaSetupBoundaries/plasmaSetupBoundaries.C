...
// The role library and the case declaration are read by
// Foam::boundaryRoleLibrary -- ONE owner, shared with
// plasmaCreateSpeciesFields. See boundaryRoleLibrary.H.
...
int main(int argc, char *argv[])
{
    ...
    {
        Info<< nl << "Boundary kinds in " << boundaryRoleLibrary::path()
            << nl << nl;

        for (const word& k : boundaryRoleLibrary::kinds())
        {
            const dictionary& r = boundaryRoleLibrary::lookup(k, "-listKinds");
    ...
    forAllConstIters(kindOfRegion, regIter)
    {
        ...
        os  << "dimensions      [1 2 -3 0 0 -1 0];" << nl << nl
            ...
            << "// and " << boundaryRoleLibrary::path() << "." << nl
        ...
        forAll(mesh.boundaryMesh(), patchi)
        {
            ...
            if (!decl.found(name))
            {
                FatalErrorInFunction
                    ...
                    << "        }" << nl << nl
                    << "    Available kinds: " << boundaryRoleLibrary::kinds() << nl
        ...
        // ---- surfCharge, on the GAS region only ---------------------------
        //
        // ENTIRELY MECHANICAL, so it is generated rather than written by hand:
        // `calculated` everywhere the field exists, and the mesh's own type on
        // the mechanical patches. The VALUES are written by
        // plasmaTransport::updateSurfaceCharge from the species wall fluxes;
        // these conditions only have to not interfere.
        //
        // Only the gas needs the file. A dielectric region's surfCharge is
        // READ_IF_PRESENT and its interface condition names the GAS side's
        // field as `surfChargeNbr`, so exactly one region owns it -- which is
        // the same one-owner rule the interface itself follows.
        if (isGas)
        {
            IOobject sio
            (
            ...
            OFstream sos(sio.objectPath());
            sio.writeHeader(sos, volScalarField::typeName);

            sos << "dimensions      [0 -2 1 0 0 1 0];" << nl << nl
                << "internalField   uniform 0;" << nl << nl
                << "// GENERATED by plasmaSetupBoundaries. Mechanical"
                   " throughout: the VALUES are" << nl
                << "// written by plasmaTransport::updateSurfaceCharge from the"
                   " species wall fluxes." << nl << nl
                << "boundaryField" << nl << "{" << nl;

            forAll(mesh.boundaryMesh(), pi)
            {
                const polyPatch& spp = mesh.boundaryMesh()[pi];

                sos << "    " << spp.name() << nl << "    {" << nl;

                word smech;
                if (mechanicalType(spp, smech))
                {
                    sos << "        type            " << smech << ";" << nl;
                }
                else
                {
                    sos << "        type            calculated;" << nl
                        << "        value           uniform 0;" << nl;
                }
            ...
            sos << "}" << nl << nl;
            sio.writeEndDivider(sos);

            Info<< "  wrote " << sio.objectPath() << endl;
            ++nWritten;
        }
    }

    Info<< nl << "plasmaSetupBoundaries: " << nWritten
        << " field(s) generated." << nl << endl;
