...
void multiRegionPoisson::solveCoupled()
{
    ...
    for (label nonOrth = 0; nonOrth <= nNonOrthCorr_; ++nonOrth)
    {
        ...
        // The one-time explicit-coupling probe that lived here has been
        // REMOVED now that it has answered: a +1 V bump of the dielectric
        // potential moved the explicit gas residual by rms 1.549 on the 160
        // interface cells against 0.0035 in the interior, so the explicit
        // fvc:: residual DOES see the neighbouring region and Newton needs no
        // lduPrimitiveMeshAssembly. Recorded in docs/CAPABILITIES.md and
        // commit 97c715b. It is gone rather than left switched off because it
        // PERTURBED and restored solution state, which does not belong in a
        // production path however carefully it is undone.
